...
if (isset($_GET["id"]) && !empty($_GET["id"]) && (func_number($_GET["id"]) == 0) && isset($_GET["valid"]) && !empty($_GET["valid"]) && ($_GET["valid"] == "oui" || $_GET["valid"] == "non"))
		{
			$id_com = $_GET["id"];
			$valid = $_GET["valid"];
			$sql = "SELECT p.id FROM photos_profil p, commentaire_profil v WHERE p.user_id='$user_id' AND p.validation=2 AND v.id_photo=p.id AND v.id=$id_com";
			$res = mysql_query($sql);
			...
			if ($num_rows == 1)
				{
					...
					if ($valid == "non")
					{
						$sql_del = "DELETE FROM commentaire_profil WHERE id=$id_com AND id_photo=$tmp[0]";
						$res_del = mysql_query($sql_del);
						$sql_updown = "UPDATE photos_profil SET commentaire= commentaire - 1 WHERE id=$tmp[0] AND user_id='$user_id'";
						$res_updown = mysql_query($sql_updown);
					}
					if ($valid == "oui")
					{
						$sql_updown = "UPDATE commentaire_profil SET validation=2 WHERE id=$id_com AND id_photo='$tmp[0]'";
						$res_updown = mysql_query($sql_updown);
					}
				}
		}
...
if (!empty($_POST["add_description"]))
{
	...
	if ($flag_test == 2)
	...
	else if ($flag_test == 0 && (func_number($id_photo) == 0))
		{
			...
			$sql = "UPDATE photos_profil SET descrip='$desc' WHERE id='$id_photo' AND user_id='$user_id'";
			$res = mysql_query($sql);
		}
	else
		{
...
<?
$sql = "SELECT nom, validation, commentaire, id, descrip FROM photos_profil WHERE user_id='$user_id'";
$res = mysql_query($sql);
$num_rows = mysql_num_rows($res);
if ($num_rows != 0)
	{
		...
		if ($tmp[1] != '2')
			...
			{
				$sql_nb_avalid = "SELECT id FROM commentaire_profil WHERE validation='1' AND id_photo=$tmp[3]";
				$res_now = mysql_query($sql_nb_avalid);
				$num_rows_now = mysql_num_rows($res_now);
				$tmp[2] -= $num_rows_now;
				$tmp[0] = stripslashes($tmp[0]);
				$tmp[4] = stripslashes($tmp[4]);
				$chemin_profil = "photos_profil/" . $tmp[0];
				echo '<form name="add_desc" method="post" action="commentaire_photos.php">';
				?>
				<input type="hidden" name="id_photo" value="<? echo $tmp[3];?>" />
				<tr><td width="250" align="left" valign="top" height="100"><? echo "<br><br><br>&nbsp;<font color=#737531><b>$tmp[2] "; if ($tmp[2] <= 1) echo "commentaire validé</b>"; else echo "<b>commentaires validés</b>"; echo "</font><br>&nbsp;<font color=#184D9C>"; if ($num_rows_now <= 1) {echo "<a href=verif_commentaire_profil.php?id=$tmp[3] class=new_comm><b>$num_rows_now  commentaire en attente</b></a>";} else {echo "<a href=verif_commentaire_profil.php?id=$tmp[3] class=new_comm><b>$num_rows_now commentaires en attente</b></a>";} echo "<br>&nbsp;<a id='entremenu' href='javascript:montrer()' class=new_desc><b>Ma description</a></b>";
				...
				?></td><td valign=bottom align="left" height="320"><a href="<? echo $chemin_profil;?>" rel="lightbox" title="<? echo $tmp[4];?>"><img src="<? echo $chemin_profil; ?>"  alt='Cliquez et faites défiler les photos' height="300" border="0" /></a></td></tr>
				...
				$sqlcomm = "SELECT p.id, p.user_id, p.id_photo, p.commentaire, p.validation, p.creation, u.user_pseudo, u.user_sex FROM commentaire_profil p, user u WHERE p.user_id=u.user_id AND p.id_photo=$tmp[3] AND validation=2 ORDER by -(p.creation)";
				...
				if ($num_comm > 0)
					{
						while ($tmp_comm = mysql_fetch_row($rescomm))
							...
							$onclick = "document.location.href='auth_profil.php?id=$tmp_comm[1]'";
							?>
							<tr><td colspan="2" align="left" class="qst_profilTitre"><? echo "&nbsp;&nbsp;Envoyé par <font color=$color>$tmp_comm[6]</font> le $str_date à $tableau_final[1]</font><font style='padding-left:150px'><a href='commentaire_photos.php?valid=non&id=$tmp_comm[0]' class=inscrip>Supprimer</font>";?></td></tr>
	<tr style="cursor:pointer" onMouseOver="this.style.background='#DEE7F7'" onMouseout="this.style.background='#f7f7f7'" onClick="<? echo $onclick;?>"><td align=left class=salutgauche width="50"><img src="<? echo $chemin;?>" height=60 width=60 alt="<? echo $tmp_comm[0];?>"></td><td align=left class=salut3 valign="top" width="90%"><? echo $tmp_comm[3];?></td></tr>
