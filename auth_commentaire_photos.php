			...
			if (isset($_POST["commentaire"]))
				{
					...
					{
						$sqltest = "SELECT u.user_pseudo, u.user_sex, u.user_mail, u.user_premium FROM user u, photos_profil p WHERE u.user_id=p.user_id AND p.id=$id_photo";
						...
						if ($numtest == 1)
							{
								...
								$sql = "INSERT INTO commentaire_profil (user_id, id_photo, commentaire, creation, validation, user_id_photo) VALUES ('".$user_id."', '".$id_photo."', '".$commentaire."', NOW(), '1', '".$visite_id."')";
								$res = mysql_query($sql);
								$tab_message = "Votre commentaire a bien été pris en compte et sera validé (ou refusé) bientôt  par " . $tmptest[0];
								$sqlupdate = "UPDATE photos_profil SET commentaire=commentaire + 1 WHERE id=$id_photo AND user_id=$visite_id";
								...
								<tr><td>Clique ici pour ' . "<a href='http://www.jlife.fr/commentaire_photos.php' class=jaimegrave>" . '<b>lire le commentaire</b></a></td></tr>
								<tr><td height="50" valign="bottom">L’&eacute;quipe de J Life,</td></tr></table>';
								mail($tmptest[2], "$tmp_pseudo[0] a ajouté un commentaire à ta photo principale", $test3, $entete);
									}
							}
						else
...
if (isset($_GET["id"]) && !empty($_GET["id"]) && (func_number($_GET["id"]) == 0) && isset($_GET["id_photo"]) && !empty($_GET["id_photo"]) && (func_number($_GET["id_photo"]) == 0))
		...
		<?
			if (empty($visite_id) || empty($id_photo))
				{
					$sqlTAKE = mysql_query("SELECT id FROM photos_profil WHERE validation=2 AND user_id='$visite_id'");
...
$sql = "SELECT nom, validation, commentaire, id, descrip FROM photos_profil WHERE user_id='$visite_id' AND id='$id_photo'";
$res = mysql_query($sql);
...
if ($num_rows != 0)
	{
		...
		if ($tmp[1] != '2')
			...
			{
				$sql_nb_avalid = "SELECT id FROM commentaire_profil WHERE validation='1' AND id_photo=$id_photo";
				$res_now = mysql_query($sql_nb_avalid);
				$num_rows_now = mysql_num_rows($res_now);
				$tmp[2] -= $num_rows_now;
				echo '<form name=commentaire_photo action=auth_commentaire_photos.php method=post>';
				...
				$sqlcomm = "SELECT p.id, p.user_id, p.id_photo, p.commentaire, p.validation, p.creation, u.user_pseudo, u.user_sex FROM commentaire_profil p, user u WHERE p.user_id=u.user_id AND p.id_photo=$id_photo AND validation=2 ORDER by -(p.creation)";
				...
				if ($num_comm > 0)
					{
						while ($tmp_comm = mysql_fetch_row($rescomm))
							...
							?>
							<tr><td colspan="2" align="left" class="qst_profilTitre"><? echo "&nbsp;&nbsp;Envoyé par <font color=$color>$tmp_comm[6]</font> le $str_date à $tableau_final[1]</font>";?></td></tr>
