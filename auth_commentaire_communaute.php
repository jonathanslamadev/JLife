			...
			if (isset($_POST["commentaire"]))
				{
					...
					{
						$sqltest = "SELECT u.user_pseudo, u.user_sex, u.user_mail, u.user_premium FROM user u, photos_communaute p WHERE u.user_id=p.user_id AND p.id=$id_photo";
						...
						if ($numtest == 1)
							{
								...
								$sql = "INSERT INTO commentaire_communaute (user_id, id_photo, commentaire, creation, validation, user_id_photo) VALUES ('".$user_id."', '".$id_photo."', '".$commentaire."', NOW(), '1', '".$visite_id."')";
								$res = mysql_query($sql);
								$tab_message = "Votre commentaire a bien été pris en compte et sera validé (ou refusé) bientôt  par " . $tmptest[0];
								$sqlupdate = "UPDATE photos_communaute SET commentaire=commentaire + 1 WHERE id=$id_photo AND user_id=$visite_id";
								...
								<tr><td>Clique ici pour ' . "<a href='http://www.jlife.fr/commentaire_communaute.php?id=$id_photo' class=jaimegrave>" . '<b>lire le commentaire</b></a></td></tr>
								<tr><td height="50" valign="bottom">L’&eacute;quipe de J Life,</td></tr></table>';
								mail($tmptest[2], "$tmp_pseudo[0] a ajouté un commentaire à ta photo communauté", $test3, $entete);
									}
							}
						else
...
if (isset($_GET["id"]) && !empty($_GET["id"]) && (func_number($_GET["id"]) == 0) && isset($_GET["id_photo"]) && !empty($_GET["id_photo"]) && (func_number($_GET["id_photo"]) == 0))
		...
		<?
			if (empty($visite_id) || empty($id_photo))
				{
					$sqlTAKE = mysql_query("SELECT id FROM photos_communaute WHERE validation=2 AND user_id='$visite_id'");
...
<tr>
<?
$sql_test = "SELECT nom, validation, id, descrip FROM photos_communaute WHERE user_id=$visite_id AND validation='2'";
...
while ($tmp = mysql_fetch_row($res_test))
{
	if ($tmp[1] == "1")
	...
	else
		{
			...
			$chemin = "photos_communaute/" . $tmp[0];
		}
	if ($i == 0)
	...
	?>
	<a href="<? echo $chemin;?>" rel="lightbox[roadtrip7]" title="<? echo $tmp[3];?>"></a>
	<?
	$onclick = "document.location.href='auth_commentaire_communaute.php?id=$visite_id&id_photo=$tmp[2]'";
		?>
	<td height=20 align=center class="<? echo $my_class;?>" style="cursor:pointer" onMouseOver="this.style.background='#299AD6'" onMouseout="this.style.background='#f7f7f7'" onClick="<? echo $onclick;?>"><img src="<? echo $chemin;?>" height=70 width=100 alt="Ajoutez un commentaire"></td>
	<?
	$i++;
if ($i == $j)
		{
...
$sql = "SELECT nom, validation, commentaire, id, descrip FROM photos_communaute WHERE user_id='$visite_id' AND id='$id_photo'";
$res = mysql_query($sql);
...
if ($num_rows != 0)
	{
		...
		if ($tmp[1] != '2')
			...
			{
				$sql_nb_avalid = "SELECT id FROM commentaire_communaute WHERE validation='1' AND id_photo=$id_photo";
				...
				echo '<form name=commentaire_photo action=auth_commentaire_communaute.php method=post>';
				$tmp[0] = stripslashes($tmp[0]);
				$tmp[4] = stripslashes($tmp[4]);
				$chemin_communaute = "photos_communaute/" . $tmp[0];
				?>
				...
				?></td><td valign=bottom align="left" height="320"><a href="<? echo $chemin_communaute;?>" rel="lightbox[roadtrip7]" title="<? echo $tmp[4];?>"><img src="<? echo $chemin_communaute; ?>"  alt='Cliquez et faites défiler les photos' border="0"></a></td></tr>
				<tr><td height="10"></td></tr>
				...
				$sqlcomm = "SELECT p.id, p.user_id, p.id_photo, p.commentaire, p.validation, p.creation, u.user_pseudo, u.user_sex FROM commentaire_communaute p, user u WHERE p.user_id=u.user_id AND p.id_photo=$id_photo AND validation=2 ORDER by -(p.creation)";
				...
				if ($num_comm > 0)
					{
						while ($tmp_comm = mysql_fetch_row($rescomm))
							...
							$onclick = "document.location.href='auth_communaute.php?id=$tmp_comm[1]'";
							?>
							<tr><td colspan="2" align="left" class="qst_comTitre"><? echo "&nbsp;&nbsp;Envoyé par <font color=$color>$tmp_comm[6]</font> le $str_date à $tableau_final[1]</font>";?></td></tr>
	<tr style="cursor:pointer" onMouseOver="this.style.background='#6BBEE7'" onMouseout="this.style.background='#f7f7f7'" onClick="<? echo $onclick;?>"><td align="left" class="salutgauche" width="50"><img src="<? echo $chemin;?>" height=60 width=60 alt="<? echo $tmp_comm[0];?>"></td><td align="left" class="salut3" valign="top" width="90%"><? echo $tmp_comm[3];?></td></tr>
