			...
			if (mysql_num_rows($result) == 1)
				{
					...
					$myNewMessage = "<form name='modif_ecole_parcours' action='fiche_ecole.php#en_bas' method='post'>" . "<b>Date d'entrée : </b>" . '<select name="mois_entree" class="select_form">';
					...
					$mySelect = "SELECT id FROM groupe_ecole WHERE nom='$nom_ecole' AND enseignement='$spec' AND user_id='$user_id'";
					$resSElect = mysql_query($mySelect);
					$tmp_fetch = mysql_fetch_row($resSElect);
					$message = "<ecole>$tmp_fetch[0]";
					...
					<tr><td>Tu as reçu un message de la part de ' . "<font color=$color_sex_ami><b>$res_premium_lol[0]</b></font>" . ' qui veut qui veut t\'inviter &agrave; son groupe.</td></tr>
...
if (!empty($_GET["spec"]) && !empty($_GET["nom"]))
	{
		$spec = $_GET["spec"];
		$nom_ecole = $_GET["nom"];
		...
		if (!empty($tmp[0]))
			{
				echo "<form name='prev_ami' action='premium_ami_ecole.php' method='POST'>";
				...
				for ($i = 1; $i != sizeof($tab_amis); $i++)
					...
					echo "<input type='hidden' value='$nom_ecole' name='nom_ecole'>";
				?>
				
				&nbsp;<b>Choisissez le destinataire dans la liste de vos amis</b></td></tr>
				<tr><td class="espace_haut" align="center">Sujet : </td><td class="espace_haut" align="left"><input id=sujet_prev type="text" name="sujet_prev" size="70" maxlength="150" value="Rejoins mon groupe <?=$spec . "&nbsp;" .  $nom_ecole;?>" onBlur="document.focus()"></td></tr>
